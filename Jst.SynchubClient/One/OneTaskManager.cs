...
{
    public class OneTaskManager
    {
        public static OneTaskManager Instance { get; private set; } = new OneTaskManager();
        // Id 
        private Dictionary<string, OneWorker> _workers = new Dictionary<string, OneWorker>();
                ...
                foreach(var item in tasks)
                {
                    if (_workers.ContainsKey(item.Id) == false)
                    {
                        try
                        {
                            ...
                            var worker = new OneWorker(item,source);
        ...
        {
            if (_workers.TryGetValue(id,out OneWorker value))
            {
                // 初始化
